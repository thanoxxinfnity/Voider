...
class NvidiaApiError extends Error {
  constructor(status, body) {
    super(`NVIDIA API error ${status}: ${body?.slice?.(0, 500) || body}`);
    this.status = status;
    this.body = body;
  }
}
...
async function invokeNim(url, payload, { timeoutMs = 15 * 60 * 1000 } = {}) {
  const headers = {
...
export async function imageTo3D(imageDataUrl, params = {}) {
  ...
  let result;
  try {
    result = await invokeNim(TRELLIS_URL, payload);
  } catch (e) {
    ...
    if (e instanceof NvidiaApiError && (e.status === 400 || e.status === 422)) {
      result = await invokeNim(TRELLIS_URL, { image: imageDataUrl });
    } else {
      throw e;
    }
  }
