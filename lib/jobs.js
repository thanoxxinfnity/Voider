...
import { imageTo3D, textTo3D, analyzePrompt } from './nvidia.js';
...
for (const j of jobs) {
  if (j.status === 'queued' || j.status === 'analyzing' || j.status === 'generating') {
    j.status = 'failed';
...
const publicJob = (j) => ({
  ...
  type: j.type,
  prompt: j.prompt,
  assetName: j.assetName || null,
  enhancedPrompt: j.enhancedPrompt || null,
  status: j.status,
  error: j.error || null,
...
async function pump() {
  ...
  try {
    ...
    if (job.type === 'image') {
      ...
      // Step 1: AI se analyse karo ki user asal me kya banwana chahta hai
      job.status = 'analyzing';
      save();
      const analysis = await analyzePrompt(job.prompt);
      job.assetName = analysis.name;
      const analyzed = analysis.prompt !== job.prompt;
      if (analyzed) job.enhancedPrompt = analysis.prompt;
      // Step 2: 3D model banao
      job.status = 'generating';
      save();
      const out = await textTo3D(analysis.prompt, { ...job.params, analyzed });
      glb = out.glb;
      if (out.viaImage) {
